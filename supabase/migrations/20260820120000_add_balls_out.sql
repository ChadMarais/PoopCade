...
values ('balls-out', 'BALLS OUT', true)
on conflict (slug) do update
set name = excluded.name,
    active = excluded.active;

-- Extend the cross-game board with BALLS OUT while preserving placement-point
-- scoring across every active game.
drop function if exists public.get_overall_leaderboard(integer);
create function public.get_overall_leaderboard(result_limit integer default 50)
returns table (
  rank bigint,
  display_name text,
  rank_points bigint,
...
as $$
  ...
  player_totals as (
    select
      ...
      max(gp.game_rank) filter (where gp.slug = 'balls-out') as balls_out_rank
